...
export default function PublicShelterDirectory() {
  ...
  const metrics = useMemo(() => {
    ...
    const totalOccupants = shelters.reduce((sum, s) => sum + (s.occupantCount || 0), 0);
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-800">
      ...
      <div className="max-w-7xl mx-auto p-6 lg:p-8">
        ...
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-extrabold text-slate-900 tracking-tight">Emergency Shelter Directory</h1>
          <p className="text-sm text-slate-600 mt-1">
            Find active emergency shelters, current occupancy rates, and direct helpline numbers.
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm">
            <p className="text-xs font-semibold text-slate-500 uppercase">Active Shelters</p>
            <p className="text-2xl font-bold text-slate-900 mt-1">{metrics.totalShelters}</p>
          </div>
          <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm">
            <p className="text-xs font-semibold text-slate-500 uppercase">Current Occupants</p>
            <p className="text-2xl font-bold text-slate-900 mt-1">{metrics.totalOccupants}</p>
          </div>
          <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm">
            <p className="text-xs font-semibold text-slate-500 uppercase">Total Capacity</p>
            <p className="text-2xl font-bold text-slate-900 mt-1">{metrics.totalCapacity}</p>
          </div>
          <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm">
            <p className="text-xs font-semibold text-slate-500 uppercase">Available Space</p>
            <p className={`text-2xl font-bold mt-1 ${metrics.availableSpace > 0 ? 'text-emerald-600' : 'text-red-600'}`}>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {displayedShelters.map((shelter) => {
              const occupancyPct = Math.round(((shelter.occupantCount || 0) / (shelter.capacity || 1)) * 100);
              const isFull = occupancyPct >= 100;

              return (
                <div key={shelter._id} className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col justify-between">
                  <div>
                    <div className="flex justify-between items-start mb-3">
                      <div>
                        <h3 className="text-lg font-bold text-slate-900">{shelter.name}</h3>
                        <p className="text-xs text-slate-500">{shelter.address || `${shelter.district}, ${shelter.division}`}</p>
                      </div>
                    ...
                    <div className="mt-4 mb-4">
                      <div className="flex justify-between text-xs font-semibold text-slate-600 mb-1">
                        <span>Occupancy</span>
                        <span>{shelter.occupantCount || 0} / {shelter.capacity || 100} ({occupancyPct}%)</span>
                      </div>
                    ...
                    <div className="border-t border-slate-100 pt-3 text-xs text-slate-600 space-y-1">
                      {shelter.managerName && <p><span className="font-semibold">Manager:</span> {shelter.managerName}</p>}
                      {shelter.contactPhone && (
                        <p>
                          <span className="font-semibold">Phone:</span>{' '}
                          <a href={`tel:${shelter.contactPhone}`} className="text-[#00ADB5] hover:underline font-bold">
                            {shelter.contactPhone}
                          </a>
                        </p>
                      )}
                      {shelter.emergencyAltPhone && (
                        <p>
                          ...
                          <a href={`tel:${shelter.emergencyAltPhone}`} className="text-slate-700 hover:underline">
                            {shelter.emergencyAltPhone}
                          </a>
                        </p>
                      )}
                  ...
                  {shelter.criticalSupplies?.some(s => s.status === 'CRITICAL' || s.status === 'LOW') && (
                    <div className="mt-4 pt-3 border-t border-slate-100">
                      <p className="text-xs font-bold text-slate-500 uppercase mb-2">Needed Supplies</p>
                          ...
                          .map((sup, idx) => (
                            <span 
                              ...
                              className={`px-2 py-0.5 rounded text-[11px] font-semibold ${
                                sup.status === 'CRITICAL' ? 'bg-red-50 text-red-700 border border-red-200' : 'bg-amber-50 text-amber-700 border border-amber-200'
                              }`}
                            >
                              {sup.category}
                            </span>
            ...
            {displayedShelters.length === 0 && (
              <div className="col-span-full py-16 text-center text-slate-500 bg-white rounded-2xl border border-slate-200">
                <p className="text-base font-semibold text-slate-700">No shelters found matching your criteria.</p>
                <p className="text-xs text-slate-400 mt-1">Try resetting your location filter or search term.</p>
              </div>
            )}
          </div>
        )}
